...
Server::Server() : tcp_service(),dispatcher(make_unique<dispatcher::ServerDispatcher>(this))
{
    ...
    auto res = sql_conn->query("select roomName from chatRoomList");
    while (res->next())
    {
        chatRoom_list.push_back(res->getString("roomName"));
    }
}
...
std::vector<std::string> Server::get_chatRoom_list()
{
...
void Server::set_user_connected_callBack(std::function<void(string&)> callback)
{
...
{
    handle_chatRoom_receive_message_callBack_ = callback;
}

void Server::on_user_connected(string& name)
{
    if (user_connect_callback_)
    {
        user_connect_callback_(name);
    }
}
