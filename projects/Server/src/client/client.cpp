    ...
    while (true)
    {
        ...
        else if (has_connect())
        {
            ...
            else if (order == "send2room")
            {
                ...
                unordered_map<string, std::variant<string, int, bool>> json_map = ix::utility::JsonInterpreter::ChatToRoom;
                json_map.at("from") = get_usr_info().username;
                json_map.at("roomName") = to;
                json_map.at("nickname") = get_usr_info().nickname;
                optional<pair<size_t,shared_ptr<vector<char>>>> totalMsg = ix::utility::JsonInterpreter::packaging(move(json_map),vector<char>(context.begin(),context.end()));
...
void Client::set_chatRoom_list(const std::vector<std::string>& room_list)
{
    chatRoom_list = room_list;
    for (auto& name : chatRoom_list)
    {
        cout << name << endl;
    }
}
